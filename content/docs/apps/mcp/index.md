...
**MCP App key (recommended)** — use the key generated by the **MCP App** (see [Setup](#setup) above).
The app auto-generates a scoped API key, so you control exactly what the AI agent can do.

**WAHA_API_KEY** — use your `WAHA_API_KEY` environment variable directly.
It gives the AI agent full access to the API, with no way to limit its permissions.
{{< /callout >}}
